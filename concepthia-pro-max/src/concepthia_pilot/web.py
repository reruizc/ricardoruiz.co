...
def docx_response(start_response: Callable, content: bytes) -> list[bytes]:
    start_response("200 OK", [
        ("Content-Type", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"),
        ("Content-Disposition", 'attachment; filename="borrador-concepthia.docx"'),
...
def app(data_dir: Path):
    def application(environ: dict[str, object], start_response: Callable) -> list[bytes]:
        ...
        # API Gateway forwards preflight requests to the $default Lambda route.
        # Return success here so a Cloudflare Pages frontend may make its POST.
        if method == "OPTIONS":
        ...
        if method != "POST" or path not in ("/api/answer", "/api/document"):
        ...
        try:
            ...
            if path == "/api/document":
                document_text = str(request.get("document", "")).strip()
                if not 3 <= len(document_text) <= 80_000:
                    return json_response(start_response, HTTPStatus.BAD_REQUEST, {"error": "El documento debe tener entre 3 y 80.000 caracteres."})
                return docx_response(start_response, create_docx(document_text))
