...
| 3 | **Publicar `estado.json` y alertar desde afuera** | `salud/PLAN-salir-del-mac.md` §5 | `run_diario.sh:494` escribe `diario/estado.json` y ahí se queda: no hay ningún `aws s3 cp` de ese archivo. Mientras el vigilante viva en la máquina vigilada, una caída no avisa. El permiso de escritura ya está en `ec2/iam-policy-caudal-cron.json` | Decidir el canal (Lambda+EventBridge exige admin; el worker `rr-auth` ya manda correo) |
