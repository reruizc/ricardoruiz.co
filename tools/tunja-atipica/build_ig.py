...
def head(ax,kick,title,sub=None,ty=None,tsize=40):
    ...
    if sub:
        sy=ty-1.02*(title.count(chr(10))+1)-0.12
        ax.text(0.7,sy,sub,fontproperties=R,fontsize=16.5,color=MUT,va='top',linespacing=1.32)
    return ty
def foot(ax,right="ricardoruiz.co/tunja-atipica.html"):
    ax.text(0.7,0.48,HANDLE,fontproperties=M,fontsize=14,color=MUT)
    ax.text(W-0.7,0.48,right,fontproperties=M,fontsize=13,color=BLUE,ha='right')
def num(ax,i):
    ax.text(W-0.7,H-0.75,f"{i:02d}/10",fontproperties=M,fontsize=13.5,color=MUT,ha='right')
...
def s02():
    ...
    for i,(yr,ape,flag,pct,col) in enumerate(rows):
        ...
        ax.text(3.0,yy+h*0.63,ape,fontproperties=M,fontsize=18,color=TXT,va='center')
        ax.text(3.0,yy+h*0.27,flag,fontproperties=R,fontsize=13,color=MUT,va='center')
        bx=W-3.5; bw=2.1
...
def map_slide(name,idx,kick,title,sub,winner_color_fn,legend):
    ...
    axm=fig.add_axes([0.02,0.15,0.96,0.44]); axm.set_facecolor(BG); axm.axis('off')
    ...
    lx=0.7
    for lab,c in legend:
        ax0.add_patch(plt.Rectangle((lx,1.05),0.26,0.26,fc=c,ec='none'))
        ax0.text(lx+0.38,1.10,lab,fontproperties=R,fontsize=13.5,color=TXT)
        lx+=4.7
...
def s04():
    ...
    for i,(ape,pct,c) in enumerate(rows):
        yy=y-i*(bh+gap)
        ax.text(x0-0.22,yy+bh/2,ape,fontproperties=R,fontsize=14,color=TXT,va='center',ha='right')
        ax.add_patch(plt.Rectangle((x0,yy),bw,bh,fc="#26304a",ec='none'))
        ax.add_patch(plt.Rectangle((x0,yy),bw*pct/mx,bh,fc=c,ec='none'))
        ax.text(x0+bw*pct/mx+0.13,yy+bh/2,f"{pct:.1f}%",fontproperties=M,fontsize=13,color=TXT,va='center')
    ...
    for i,(big,small) in enumerate(facts):
        ...
        ax.text(xx+0.32,1.38,small,fontproperties=R,fontsize=12,color=MUT)
...
def s05():
    ...
    for i,(nm,pct,c) in enumerate(rows):
        yy=y-i*(bh+gap)
        ax.text(x0,yy+bh+0.12,nm,fontproperties=M,fontsize=17,color=TXT)
        ax.add_patch(plt.Rectangle((x0,yy),bw,bh,fc="#26304a",ec='none'))
    ...
    ax.text(0.7,2.05,f"1ª vuelta: Cepeda {100*c1/(c1+a1):.1f}%    ·    participación 2V {100*u2/pot:.1f}%",
            fontproperties=R,fontsize=15,color=MUT)
    ax.text(0.7,1.45,"El techo de la izquierda es alto. La pregunta de la atípica es si\nese voto se traduce en un solo candidato local.",
            fontproperties=R,fontsize=14.5,color=MUT,linespacing=1.3)
...
def s07():
    ...
    for i,(par,nm,c) in enumerate(blocs):
        ...
        ax.text(1.1,yy+h*0.62,nm,fontproperties=M,fontsize=17,color=TXT,va='center')
        ax.text(1.1,yy+h*0.26,par,fontproperties=R,fontsize=12.5,color=MUT,va='center')
    ax.text(0.7,1.15,"Sumados valen más que cualquiera por separado. Divididos, le abren\nla puerta al bloque tradicional y a los independientes.",
            fontproperties=R,fontsize=14,color=MUT,linespacing=1.3)
...
def s08():
    ...
    for i,(nm,par,c) in enumerate(cands):
        ...
        ax.text(2.15,yy+h*0.62,nm,fontproperties=B,fontsize=19,color=TXT,va='center')
        ax.text(2.15,yy+h*0.27,par,fontproperties=R,fontsize=13,color=MUT,va='center')
...
def s09():
    ...
    for t in pts:
        ...
        ax.text(1.4,y,t,fontproperties=R,fontsize=16,color=TXT,va='center',wrap=True,linespacing=1.28)
        y-=1.45
...
def s10():
    ...
    ax.text(0.7,H-4.15,"Las 4 alcaldías (2011–2023) y la presidencial 2026, gratis.\nHistórico completo, capa de puestos y base descargable\nen los planes.",
            fontproperties=R,fontsize=16,color=MUT,va='top',linespacing=1.35)
