#!/usr/bin/env python3
"""
build_congreso_2018.py — genera los JSONs por candidato del CONGRESO 2018-2022
(Senado + Cámara) en el MISMO formato mesa-a-mesa de endoso/{slug}.json que
consume analisis-candidato.html, más un index-congreso-2018.json.
...
Fuentes (locales):
  Bases de datos/FINAL SUBIDA GCS/GCS_2018CON.csv   (1,28 GB)
  Bases de datos/PUESTOS_GEOREF.csv                 (nombres mun/puesto/comuna)

Salida (gitignored, ~1 GB):
  Bases de datos/output_congreso_2018/
    index-congreso-2018.json
    CON2018-S-{par}-{can}.json          senado nacional
    CON2018-SI-{par}-{can}.json         senado indígena
    CON2018-C-{dde}-{par}-{can}.json    cámara territorial
    CON2018-CA-{par}-{can}.json         cámara afro
    CON2018-CI-{par}-{can}.json         cámara indígena

Subida a S3 (manual, pedir luz verde):
  aws s3 cp "Bases de datos/output_congreso_2018/" \
    "s3://elecciones-2026/ricardoruiz.co/congreso-2026/output/congreso-2018/" \
    --recursive --content-type "application/json" --cache-control "public, max-age=300"

Uso:
  python3 tools/analisis-candidato/build_congreso_2018.py [--limit N]
"""
...
SRC = os.path.join(BD, 'FINAL SUBIDA GCS', 'GCS_2018CON.csv')
GEOREF = os.path.join(BD, 'PUESTOS_GEOREF.csv')
OUT_DIR = os.path.join(BD, 'output_congreso_2018')
...
def main():
    ...
    for (pref, dde, par, can), c in cands.items():
        slug = f'CON2018-{pref}-{par}-{can}' if c['nacional'] else f'CON2018-{pref}-{dde}-{par}-{can}'
