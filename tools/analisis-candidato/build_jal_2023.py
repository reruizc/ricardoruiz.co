#!/usr/bin/env python3
"""
build_jal_2023.py — genera los JSONs por candidato de JAL (Junta Administradora
Local · EDIL) 2023 en el MISMO formato mesa-a-mesa de endoso/{slug}.json que
consume analisis-candidato.html / comparar-candidatos.html, + index-jal-2023.json.

JAL es circunscripción LOCAL: cada candidato compite en UNA comuna/localidad de un
municipio. ⚠️ El número de candidato (COD_CAN) SE REINICIA por comuna — CAN=81
del partido 1 es una persona distinta en cada comuna. Por eso la llave incluye el
NOMBRE: (dde, mme, par, can, nombre_norm). El slug agrega un hash del nombre para
unicidad estable. La comuna/localidad se deriva del georef (moda por votos).

Fuentes (locales):
  Bases de datos/FINAL SUBIDA GCS/GCS_2023JAL.csv   (COD_COR='5' = JAL · 2,25M filas)
  Bases de datos/PUESTOS_GEOREF.csv                 (nombres mun/puesto/comuna)

Salida (gitignored):
  Bases de datos/output_jal_2023/
    index-jal-2023.json
    JAL2023-{dde}-{mme}-{par}-{can}-{hash6}.json   (~13,6k)

Subida a S3 (manual, luz verde del usuario):
  aws s3 cp "Bases de datos/output_jal_2023/" \
...
OUT_DIR = os.path.join(BD, 'output_jal_2023')
...
def ensure_sorted():
    ...
    print('· ordenando GCS_2023JAL por (dde,mme)…')
    awk = ["awk", "-F;", 'NR>1', SRC]
...
def flush_group(cands, by9, munNames, index):
    for (dde, mme, par, can, nkey), c in cands.items():
        ...
        # Bogotá D.C.: mme=001 para toda la ciudad; georef guarda la localidad en
        # MUNICIPIO. El nombre de localidad ya viaja aparte en comNom.
        munNom = 'BOGOTÁ D.C.' if dd == '16' else munNames.get(f'{dd}{mm}', c['munNom'] or f'MUN {mm}')
        # comuna/localidad dominante (moda por votos)
        comNom = 'LOCAL'
        if c['comVotes']:
        ...
        slug = f'JAL2023-{dde}-{mme}-{par}-{can}-{h6}'
        circ = f'{comNom} · {munNom}'
        data = {
            'nombre': c['nombre'], 'corp': f'JAL · {comNom} · {munNom}',
...
def main():
    ...
    with open(SORTED, encoding='utf-8', errors='replace', newline='') as f:
        for row in csv.reader(f, delimiter=';'):
            ...
            if v <= 0:
                continue
            dde = row[C_DDE].strip(); mme = row[C_MME].strip()
            grp = (dde, mme)
            if grp != cur_group:
            ...
            if c is None:
                c = cands[key] = {
                    ...
                    'munNom': None, 'votos': 0, 'mesas': [], 'comVotes': {},
                }
            dd = dde.zfill(2); mm = mme.zfill(3)
    ...
    print('Top 5:', [(x['nombre'][:20], x['circunscripcion'][:26], fmt(x['votos'])) for x in index[:5]])
