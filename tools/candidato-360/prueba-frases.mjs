...
/* Por firmas la frase NO puede hablar del aval viejo como si fuera el nuevo:
   el caso de Carlos Mario Bedoya, que se lanzó con MAIS y ahora va por firmas
   ubicándose en la derecha. */
r.firmas = await frase({ candidate: { nombre: 'CARLOS MARIO BEDOYA MORENO', corp: 'ALCALDÍA · LA CEJA · 2023', partido: 'MOVIMIENTO ALTERNATIVO INDÍGENA Y SOCIAL "MAIS"' },
  corpKey: 'alcaldia', territory: 'LA CEJA · Antioquia', campana: { corp: 'alcaldia', avales: 'firmas', espectro: 'd', departamento: '01' } });
r.firmasSinHistorialDePartido = await frase({ candidate: { nombre: 'OTRA MÁS', corp: 'ALCALDÍA · LA CEJA · 2023' },
  corpKey: 'alcaldia', territory: 'LA CEJA · Antioquia', campana: { corp: 'alcaldia', avales: 'firmas', espectro: 'izq', departamento: '01' } });
await b.close();

const pruebas = [
  ['por firmas la frase habla de firmas y de la familia elegida, no del aval viejo', /por firmas/i.test(r.firmas) && /la derecha/.test(r.firmas) && !/Con Alternativo/i.test(r.firmas)],
  ['y el aval anterior se nombra como pasado, con su sigla', /Viene de Alternativo Indígena y Social "MAIS", pero esta vez/.test(r.firmas)],
  ['sin partido anterior, la frase por firmas arranca bien igual', /^[A-ZÁÉÍÓÚ¡]/.test(r.firmasSinHistorialDePartido.split('. ').pop()) && /la izquierda/.test(r.firmasSinHistorialDePartido)],
