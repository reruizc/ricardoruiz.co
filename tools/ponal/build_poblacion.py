...
def main():
    ...
    # A=DP  B=DPNOM  C=MPIO  D=DPMP  E=AÑO  F=ÁREA  G=Total
    muns = defaultdict(lambda: {"n": "", "p": {}})
    deps = defaultdict(lambda: {"n": "", "p": defaultdict(int)})
    nac = defaultdict(int)
    filas = usadas = 0
    ...
    out = {
        ...
        "muns": {k: {"n": v["n"], "p": [v["p"].get(a, 0) for a in ANIOS]} for k, v in sorted(muns.items())},
        "deptos": {k: {"n": v["n"], "p": [v["p"].get(a, 0) for a in ANIOS]} for k, v in sorted(deps.items())},
    ...
    for a, p in zip(ANIOS, out["nacional"]):
        print(f"    {a}  {p:>12,}")
